...
export function createNode(kind, label, position) {
    ...
    const data = kind === 'agent' || kind === 'code' ? { kind, label, timeout: '120' } : { kind, label };
    return {
...
export function toStepList(doc) {
    ...
    for (const n of doc.nodes) {
        if (n.data.kind === 'trigger' || n.data.kind === 'schedule')
            emit(n.id);
    }
    return out;
}
 ...
 */
export function buildWeatherExample() {
    const nodes = [
        {
            id: 'w-trigger',
            type: 'workflow',
...
export function buildNewsExample() {
    const nodes = [
...
export function buildBasicIfExample() {
    const nodes = [
...
export function buildQaIfExample() {
    const nodes = [
...
export function buildDailySwitchExample() {
    const nodes = [
...
export function buildLoopExample() {
    const nodes = [
...
export function buildWhileExample() {
    const nodes = [
