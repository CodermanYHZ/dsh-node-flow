...
import type { WorkflowDocument, WorkflowEdge, WorkflowNode, WorkflowNodeData } from './model.js'
export const name = 'dsh-node-flow'
export const inject = ['webServer'] as const

/** Minimal structural view of DSH's `ctx.codeRuntime` (see `@deepseek-ai/dsh-code-runtime`). */
interface CodeRuntimeLike {
  readonly language: string
  run(request: { program: string; bindings: unknown[]; signal?: AbortSignal }): Promise<{
    value?: unknown
    logs: string[]
...
/** Minimal structural view of DSH's `ctx.webServer` route registration. */
...
/** Minimal structural view of DSH's `ctx.agents` service (see `@deepseek-ai/dsh-agent`). */
interface AgentLike {
  readonly id: unknown
}
...
/** Minimal structural view of DSH's `ctx.subagents` result/run types. */
...
/** Minimal structural view of DSH's `ctx.llm` (LlmRuntime) model directory. */
interface LlmLike {
...
function outputText(result: SubagentResultLike): string {
...
function pickSubagentProvider(subagents: SubagentsLike): string | undefined {
  for (const name of ['spawn', 'fork', 'acp']) {
    if (subagents.getProvider(name) !== undefined) return name
  }
 ...
 */
function buildCodeBindings(input: unknown): unknown[] {
  return [
    {
      global: 'dsh',
      functions: {
        fetch: async (args: unknown) => {
          ...
          const headers =
            a.headers !== null && typeof a.headers === 'object'
...
function runPythonCode(
  ...
  input: unknown,
  signal?: AbortSignal,
): Promise<{ readonly value?: unknown; readonly logs: string[]; readonly error?: { kind: string; message: string } }> {
  const inputStr =
    typeof input === 'string'
      ? input
  ...
  return new Promise((resolve) => {
    ...
    const tryRun = (i: number): void => {
      ...
      execFile(cmd, ['-u', '-c', code], { env, timeout: 60_000, maxBuffer: 10 * 1024 * 1024, signal }, (error, stdout, stderr) => {
        ...
        if (error) {
          const aborted = (error as NodeJS.ErrnoException & { name?: string }).name === 'AbortError'
...
function rejectNonPost(request: IncomingMessage, response: ServerResponse): boolean {
 ...
 */
function makeTimeoutSignal(timeoutSec: unknown): { signal: AbortSignal; seconds: number; isTimeout: () => boolean; clear: () => void } {
  const controller = new AbortController()
  let timedOut = false
...
/* ---------------- scheduled runs (lightweight cron + host executor) ---------------- */
...
interface ScheduleRecord {
  ...
  input?: string
  steps: unknown[]
  /** Full workflow graph captured at save time (used to execute branch/loop). */
...
function loadSchedules(): ScheduleRecord[] {
  return readJson<ScheduleRecord[]>(schedulesFile(), [])
}
function saveSchedules(list: ScheduleRecord[]): void {
...
function parseCronField(field: string, min: number, max: number): (v: number) => boolean {
...
function cronMatches(cron: string, date: Date): boolean {
  ...
  return (
    parseCronField(f[0], 0, 59)(date.getMinutes()) &&
 ...
 */
function utcMinuteKeyToLocal(key: string): string {
...
function evalExprHost(expr: string, result: unknown): unknown {
  try {
...
function evalConditionHost(expr: string, result: unknown): boolean {
  return Boolean(evalExprHost(expr, result))
}
function renderTemplateHost(template: string, vars: Record<string, unknown>): string {
...
function parseAgentOptions(model: string | undefined): { provider?: string; model?: string } | undefined {
 ...
 */
async function runDocumentGraph(ctx: Context, doc: WorkflowDocument, initialInput: unknown): Promise<RunRecord> {
  const docNodes: WorkflowNode[] = Array.isArray(doc?.nodes) ? doc.nodes : []
  const docEdges: WorkflowEdge[] = Array.isArray(doc?.edges) ? doc.edges : []
  const byId = new Map(docNodes.map((n) => [n.id, n]))
  const edgesFrom = (id: string): WorkflowEdge[] => docEdges.filter((e) => e.source === id)
  ...
  const recFor = (id: string): NodeRunRecord => {
  ...
  const executeNode = async (id: string, input: string, ctxVisited: Set<string>, stopAt?: string): Promise<string> => {
    ...
    const rec = recFor(id)
    const t0 = Date.now()
    const setRec = (patch: Partial<NodeRunRecord>): void => {
    ...
    const fanOut = async (value: string): Promise<string> => {
      let last = value
      for (const e of edgesFrom(id)) last = await executeNode(e.target, value, ctxVisited, stopAt)
      return last
    }
      ...
      if (d.kind === 'trigger' || d.kind === 'schedule') {
        ...
        try {
          if (d.language === 'python') {
            ...
            const codeRuntime = ctx.get('codeRuntime') as CodeRuntimeLike | undefined
            if (!codeRuntime) nodeError = 'codeRuntime unavailable on this host'
            else {
        ...
        const ts = makeTimeoutSignal(d.timeout)
        try {
          if (!agents || !subagents) nodeError = 'subagent services unavailable on this host'
          else {
            const parent = agents.roots()[0] ?? agents.list()[0]
            const provider = pickSubagentProvider(subagents)
            if (!parent) nodeError = 'no live agent to delegate from'
            else if (!provider) nodeError = 'no subagent provider registered'
            else {
              ...
              const result: SubagentResultLike = await run.result
        ...
        if (edge) return await executeNode(edge.target, input, ctxVisited, stopAt)
        return input
        ...
        if (doneEdge) return await executeNode(doneEdge.target, value, ctxVisited, stopAt)
        return value
      }
      return input
    } catch (err) {
  ...
  for (const sid of starts) await executeNode(sid, '', visited)
  if (!ran && records.size === 0) {
    const rec: NodeRunRecord = { id: '', kind: 'none', status: 'ok', output: 'no runnable nodes', ms: 0 }
    records.set('__none__', rec)
  }

  const nodes: NodeRunRecord[] = [...records.values()]
...
function nextCronFire(cron: string, from: Date): Date | null {
  const f = cron.trim().split(/\s+/)
  ...
  for (let dayOffset = 0; dayOffset < 366 * 2; dayOffset++) {
    ...
    if (!monP(day.getMonth() + 1)) continue
    if (!dayP(day.getDate())) continue
    if (!dowP(day.getDay())) continue
...
function rescheduleScheduler(ctx: Context): void {
  if (scheduleTimer) {
  ...
  for (const s of schedules) {
    ...
    if (t && (next === null || t.getTime() < next.getTime())) next = t
  }
  if (next === null) return
...
function runDueSchedules(ctx: Context): void {
  const now = new Date()
  const minuteKey = now.toISOString().slice(0, 16)
  const schedules = loadSchedules()
  for (const sched of schedules) {
    if (sched.lastFired === minuteKey) continue
    ...
    disposers.push(
      webServer.register({
        ...
        handler: async (request, response) => {
          if (rejectNonPost(request, response)) return
          if (!sameOrigin(request)) return sendJson(response, 403, { error: 'untrusted origin' })
          let ts: ReturnType<typeof makeTimeoutSignal> | undefined
          try {
            const body = (await readJsonBody(request, 256 * 1024)) as
              | { program?: unknown; language?: unknown; input?: unknown; timeout?: unknown }
              | undefined
    ...
    disposers.push(
      webServer.register({
        ...
        handler: async (request, response) => {
          ...
          try {
            const body = (await readJsonBody(request, 256 * 1024)) as
              | { prompt?: unknown; timeout?: unknown; agentOptions?: unknown }
              | undefined
            ...
            if (typeof body?.agentOptions === 'object' && body?.agentOptions !== null) {
              const ao = body.agentOptions as { provider?: unknown; model?: unknown }
    ...
    disposers.push(
      webServer.register({
        ...
        handler: async (request, response) => {
          ...
          try {
            const body = (await readJsonBody(request, 256 * 1024)) as
              | { action?: unknown; id?: unknown; name?: unknown; cron?: unknown; input?: unknown; steps?: unknown; doc?: unknown }
              | undefined
            const schedules = loadSchedules()
            ...
            if (body?.action === 'run') {
              ...
              if (!sched) return sendJson(response, 404, { error: 'schedule not found' })
              if (!sched.doc) return sendJson(response, 400, { error: 'schedule has no workflow snapshot' })
            ...
            const doc = body?.doc && typeof body.doc === 'object' ? (body.doc as WorkflowDocument) : undefined
